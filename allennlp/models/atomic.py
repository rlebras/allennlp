...
# from event2mind import StateDecoder as StateDecoderE2M
...
class StateDecoder:
    ...
    def sample_from_topk(self,
        ...
        for i in range(num_decoding_steps):
            ...
            logits_flat = output_projection_layer(emb_out_flat)
            probs_flat = F.softmax(logits_flat, dim=-1)
            
            vals, indx = probs_flat.topk(k)
            next_indx_indx = torch.multinomial(vals, 1)
            next_tok_i = indx.gather(-1, next_indx_indx).squeeze()
            next_tok_lp = torch.log(vals.gather(-1, next_indx_indx)).squeeze()
        ...
        pred_token_ps = torch.stack(pred_token_ps,dim=-1)# .unsqueeze(0)
        pred_token_ps = pred_token_ps.cumsum(dim=2)
        
        # Moment when cumsum = 1: the first end token
        mask = ((pred_token_idx == end_ix).cumsum(dim=2) == 1).type_as(pred_token_idx)
        prob_idxs = mask.argmax(dim=2)
        pred_token_ps_end = pred_token_ps.gather(-1, prob_idxs.unsqueeze(2)).squeeze()

        return pred_token_idx, pred_token_ps_end

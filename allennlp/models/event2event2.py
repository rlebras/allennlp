...
@Model.register("event2event2")
class Event2Event2(Model):
    """
    This ``Event2Event2`` class is a :class:`Model` which takes an event
    ...
    def __init__(self,
        ...
        super(Event2Event2, self).__init__(vocab)
        ...
        self._states: Dict[str, Event2Event2.StateDecoder] = {}
    ...
    class StateDecoder:
        def __init__(self, name, event2event2, num_classes, input_dim, output_dim):
            self._embedder = Embedding(num_classes, input_dim)
            event2event2.add_module("{}_embedder".format(name), self._embedder)
            self._decoder_cell = GRUCell(input_dim, output_dim)
            event2event2.add_module("{}_decoder_cell".format(name), self._decoder_cell)
            self._output_projection_layer = Linear(output_dim, num_classes)
            event2event2.add_module("{}_output_project_layer".format(name), self._output_projection_layer)

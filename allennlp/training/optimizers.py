...
@Optimizer.register('openai_adam')
class WeightDecayAdamOptimizer(torch.optim.Optimizer):

    def __init__(self, params, lr, warmup, t_total,
        ...
        defaults = dict(lr=lr, schedule="warmup_linear", warmup=warmup, t_total=t_total,
                        b1=b1, b2=b2, e=e, l2=l2, vector_l2=vector_l2,
                        max_grad_norm=max_grad_norm)
        super(WeightDecayAdamOptimizer, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                lr_scheduled = group['lr'] * warmup_linear(state['step']/group['t_total'], group['warmup'])
